...
class DD32 {
	...
	function add_changelog_rows($plugin, $url, $plugin_data) {
		...
		if ( ! isset($changelogs[$url]) || !isset($changelogs[$url]['time']) || $changelogs[$url]['time'] < time()-24*60*60 ) {
			$log = wp_remote_get($url, array('timeout' => 50));
			if ( is_wp_error($log) || $log['response']['code'] != 200 ) {
				echo '<tr><td colspan="5" class="plugin-update">Error during loading changelogs: ' . (is_wp_error($log) ? $log->get_error_message() : $log['response']['code'] . ' '. $log['response']['message']) . '</td></tr>';
				return;
			}
